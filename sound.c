...
GameSoundData GameSound_Init()
{
    ...
    if (!SDL_LoadWAV("1-02 - Moon in One's Cup.wav", &wav_spec, &wav_data, &wav_data_len))
    { // загружає аудіо-файл
    ...
    if (!SDL_PutAudioStreamData(stream, wav_data, wav_data_len))
    { // кладемо аудіо-файл в стрім(приймач цього файла)
...
void FlappingSound()
{
    ...
    if (!SDL_LoadWAV("dragon-studio-dragon-flapping-wings-364476_WIbR3fWD.wav", &wav_spec, &wav_data, &wav_data_len))
    { // загружає аудіо-файл
        SDL_Log("Couldn't initialize WAV: %s", SDL_GetError());
